...
IF ( UNIX AND NOT APPLE AND NOT DISABLE_GOLD AND NOT DEFINED CMAKE_LINKER_TYPE )
    ...
    IF ( "${DEFAULT_COMPILER_ID}" MATCHES "GNU" )
        SET( LINKER_TYPE_FLAGS "-fuse-ld=gold -Wl,--disable-new-dtags -Wl,--no-map-whole-files" )
        CHECK_LINKER_FLAG( CXX "${LINKER_TYPE_FLAGS}" test_gold )
        IF ( test_gold )
            MESSAGE( "Using gold linker" )
            SET( CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} ${LINKER_TYPE_FLAGS}" )
            SET( CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${LINKER_TYPE_FLAGS}" )
        ENDIF()
    ELSEIF ( "${DEFAULT_COMPILER_ID}" MATCHES "(Clang|LLVM)" )
        SET( LINKER_TYPE_FLAGS "-fuse-ld=lld" )
        CHECK_LINKER_FLAG( CXX "${LINKER_TYPE_FLAGS}" test_lld )
        IF ( test_lld )
