...
#define PROFILE_START(NAME,LEVEL) call GLOBAL_PROFILE_START(NAME,__FILE__,__LINE__,LEVEL)
...
#define PROFILE_STOP(NAME,LEVEL) call GLOBAL_PROFILE_STOP(NAME,__FILE__,LEVEL)
...
#define PROFILE_SYNCHRONIZE() call GLOBAL_PROFILE_SYNC()


c PROFILE_SAVE(FILE,GLOBAL)
c This will save the results of the timers the file provided
c An optional argument specifying the level to enable may be included.
#define PROFILE_SAVE(FILE) call GLOBAL_PROFILE_SAVE(FILE,1) 


c PROFILE_ENABLE(LEVEL)
c This will enable the timers.
c An optional argument specifying the level to enable may be included.
#define PROFILE_ENABLE(LEVEL) call GLOBAL_PROFILE_ENABLE(LEVEL)


c PROFILE_DISABLE()
c This will disable the timers.
#define PROFILE_DISABLE() call GLOBAL_PROFILE_DISABLE()
...
#define PROFILE_ENABLE_TRACE() call GLOBAL_PROFILE_SET_STORE_TRACE(1)
...
c By default trace cabailities are disabled and may affect the
c performance if enabled.
#define PROFILE_DISABLE_TRACE() call GLOBAL_PROFILE_SET_STORE_TRACE(0)


c PROFILE_ENABLE_MEMORY()
c This will enable the monitoring of the memory usage within the application. 
#define PROFILE_ENABLE_MEMORY() call GLOBAL_PROFILE_SET_STORE_MEMORY(1)


c PROFILE_DISABLE_MEMORY()
c This will disable the monitoring of the memory usage within the application. 
#define PROFILE_DISABLE_MEMORY() call GLOBAL_PROFILE_SET_STORE_MEMORY(0)
