...
constexpr void getRGB( uint32_t rgb, unsigned char& r, unsigned char& g, unsigned char& b )
{
    r = ( rgb >> 16 ) & 0xFF;
    g = ( rgb >> 8 ) & 0xFF;
    b = rgb & 0xFF;
...
constexpr uint32_t getRGB( unsigned char r, unsigned char g, unsigned char b )
{
...
constexpr uint32_t jet( double value )
{
    uint r = 0, g = 0, b = 0;
    if ( value < 0 ) {
        r = g = b = 0;
