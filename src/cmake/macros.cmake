...
FUNCTION( ADD_PROJ_PROVISIONAL_TEST EXEFILE )
    ...
    SET( LAST_TEST_EXE ${EXE} PARENT_SCOPE )
ENDFUNCTION()
...
FUNCTION( ADD_${PROJ}_TEST EXEFILE ${ARGN} )
    ...
    IF ( USE_MPI_FOR_SERIAL_TESTS )
        ADD_TEST( ${TESTNAME} ${MPIEXEC} ${MPIEXEC_NUMPROC_FLAG} 1 ${LAST_TEST_EXE} ${ARGN} )
        SET_PROPERTY( TEST ${TESTNAME} APPEND PROPERTY ENVIRONMENT OMPI_MCA_hwloc_base_binding_policy=none )
    ELSE()
        ADD_TEST( ${TESTNAME} ${LAST_TEST_EXE} ${ARGN} )
    ENDIF()
...
FUNCTION( ADD_${PROJ}_WEEKLY_TEST EXEFILE PROCS ${ARGN} )
    ...
    ELSEIF( (USE_MPI OR USE_EXT_MPI) AND NOT (${PROCS} GREATER ${TEST_MAX_PROCS}) )
        CREATE_TEST_NAME( "${EXEFILE}_${PROCS}procs_WEEKLY" ${ARGN} )
        ADD_TEST( ${TESTNAME} ${MPIEXEC} ${MPIEXEC_NUMPROC_FLAG} ${PROCS} ${LAST_TEST_EXE} ${ARGN} )
...
FUNCTION( ADD_${PROJ}_EXAMPLE EXEFILE PROCS ${ARGN} )
    ...
    ADD_CUSTOM_COMMAND( TARGET ${EXEFILE} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy ${LAST_TEST_EXE} "${EXAMPLE_INSTALL_DIR}/${EXEFILE}"
    )
...
MACRO( ADD_MATLAB_TEST EXEFILE ${ARGN} )
    FIND_PROGRAM( MATLAB_EXE NAME matlab PATHS "${MATLAB_DIRECTORY}/bin" NO_DEFAULT_PATH ) 
    ...
    ENDIF()
    SET( MATLAB_COMMAND "addpath('${${PROJ}_BINARY_DIR}/matlab'); try, ${EXEFILE}, catch ME, ME, clear all global, exit(1), end, disp('ALL TESTS PASSED'); exit(0)" )
    ...
    ELSE()
        ADD_TEST( ${TESTNAME} "${MATLAB_EXE}" ${MATLAB_OPTIONS} -r "${MATLAB_COMMAND}" ${MATLAB_DEBUGGER_OPTIONS} )
    ENDIF()
