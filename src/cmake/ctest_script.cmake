...
IF( ${HOSTNAME} STREQUAL "lap0086227" )
    ...
    SET( CXX "mpicxx" )
    SET( MPIEXEC "mpirun" )
    SET( RATES_DIRECTORY  "/projects/FAC/rates_v28" )
    SET( MATLAB_DIRECTORY "/usr/local/matlab" )
    SET( USE_ACML TRUE )
    SET( ACML_DIRECTORY   "/packages/acml-4-4-0/gfortran64" )
...
ELSEIF( ${HOSTNAME} MATCHES "oblivion.*" ) 
    ...
    SET( CXX "mpicxx" )
    SET( MPIEXEC "mpirun" )
    SET( RATES_DIRECTORY  "/packages/FAC/rates_v28" )
    SET( MATLAB_DIRECTORY "/usr/MATHWORKS_R2008A" )
    SET( USE_ACML TRUE )
    SET( ACML_DIRECTORY   "/packages/acml-4-4-0-gfortran-64bit/gfortran64" )
    SET( COVERAGE_COMMAND /usr/bin/gcov )
    SET( VALGRIND_COMMAND /usr/bin/valgrind )
    SET( LDLIBS "-lmpi" )
    SET( CTEST_CMAKE_GENERATOR "Unix Makefiles" )
ELSEIF( ${HOSTNAME} MATCHES "GAMING" ) 
    SET( MPIEXEC "mpiexec" )
    SET( RATES_DIRECTORY  "C:/nightly_builds/rates" )
    SET( MATLAB_DIRECTORY "C:/Program Files/MATLAB/R2013a" )
    SET( USE_MKL TRUE )
    SET( MKL_DIRECTORY   "C:/Program Files (x86)/Intel/Composer XE 2013 SP1/mkl" )
    SET( MAKE_CMD nmake )
...
ELSEIF( ${CTEST_SCRIPT_ARG} STREQUAL "debug" )
    SET( CTEST_BUILD_NAME "TIMERModel-debug" )
...
ELSEIF( (${CTEST_SCRIPT_ARG} STREQUAL "optimized") OR (${CTEST_SCRIPT_ARG} STREQUAL "opt") )
    SET( CTEST_BUILD_NAME "TIMERModel-opt" )
...
ELSEIF( ${CTEST_SCRIPT_ARG} STREQUAL "valgrind" )
    SET( CTEST_BUILD_NAME "TIMERModel-valgrind" )
    SET( CMAKE_BUILD_TYPE "Debug" )
    SET( CTEST_COVERAGE_COMMAND )
    SET( ENABLE_GCOV "false" )
...
ELSEIF( ${CTEST_SCRIPT_ARG} STREQUAL "valgrind-matlab" )
    SET( CTEST_BUILD_NAME "TIMERModel-valgrind-matlab" )
    SET( CMAKE_BUILD_TYPE "Debug" )
    SET( CTEST_COVERAGE_COMMAND )
    SET( ENABLE_GCOV "false" )
    SET( USE_VALGRIND FALSE )
...
SET( CTEST_OPTIONS "${CTEST_OPTIONS};-DRATES_DIRECTORY='${RATES_DIRECTORY}'" )
IF ( USE_ACML ) 
