...
QSplitterGrid::QSplitterGrid( QWidget* parent ):
    QScrollArea(parent), grid(NULL), hspacing(1), vspacing(1),
    row_uniform(false), col_uniform(false), frame_widget(NULL)
{
    reset();
}
...
{
    for (auto ptr : row_boundaries) { delete ptr; }
    for (auto ptr : col_boundaries) { delete ptr; }
...
void QSplitterGrid::tableSize( int N_rows, int N_columns )
{
    ...
    for (size_t i=0; i<row_boundaries.size(); i++)
        row_boundaries[i] = new QSplitterGridLineClass(Qt::SizeVerCursor,i+1,frame_widget,this);
    col_boundaries.resize(N_columns);
    for (size_t i=0; i<col_boundaries.size(); i++)
        col_boundaries[i] = new QSplitterGridLineClass(Qt::SizeHorCursor,-(i+1),frame_widget,this);
}
...
void QSplitterGrid::moveBoundary( int index0, int pos, bool final )
{
    if ( index0 < 0 ) {
        ...
        int index = -index0-1;
        int p0, shift;
        calcMoveBoundaryShift(index,pos,vspacing,col_uniform,col_size,min_col_size,p0,shift);
        ...
        int index = index0-1;
        int p0, shift;
        calcMoveBoundaryShift(index,pos,hspacing,row_uniform,row_size,min_row_size,p0,shift);
